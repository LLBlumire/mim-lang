//! Contains the data section processing functions
...
pub fn data(
    ...
    // Process stored unsigned numbers
    context_rule!(
        term,
        buf,
        nametable,
        "number",
        0,
        |term: &str, (buf, added): (&mut Vec<u8>, i64)| {
            for _ in 0..added {
                buf.pop();
            }
            let mut params = term.split_whitespace()
                .skip_while(|n| &n[n.len() - 1..] == ":")
                .skip(1);
            if let Some(Some(base)) = params.next().map(|n| n.parse::<u32>().ok()) {
                if let Some(bytes) = params.next() {
                    if let Some(number) = params.next() {
                        ...
                        match bytes {
                            "1" | "size8" => buf.write_u8(
                                u8::from_str_radix(number, base).unwrap_or_else(|_| err!()),
                            ),
                            "2" | "size16" => buf.write_u16::<BE>(
                                u16::from_str_radix(number, base).unwrap_or_else(|_| err!()),
                            ),
                            "4" | "size32" => buf.write_u32::<BE>(
                                u32::from_str_radix(number, base).unwrap_or_else(|_| err!()),
                            ),
                            "8" | "size64" => buf.write_u64::<BE>(
                                u64::from_str_radix(number, base).unwrap_or_else(|_| err!()),
                            ),
                            _ => {
            ...
            if let Some(Some(base)) = params.next().map(|n| n.parse::<u32>().ok()) {
                if let Some(bytes) = params.next() {
                    if let Some(number) = params.next() {
                        ...
                        match bytes {
                            "1" | "size8" => buf.write_i8(
                                i8::from_str_radix(number, base).unwrap_or_else(|_| err!()),
                            ),
                            "2" | "size16" => buf.write_i16::<BE>(
                                i16::from_str_radix(number, base).unwrap_or_else(|_| err!()),
                            ),
                            "4" | "size32" => buf.write_i32::<BE>(
                                i32::from_str_radix(number, base).unwrap_or_else(|_| err!()),
                            ),
                            "8" | "size64" => buf.write_i64::<BE>(
                                i64::from_str_radix(number, base).unwrap_or_else(|_| err!()),
                            ),
                            _ => {
                                println!("Unknown size align for term: `{}`", term);
                                buf.write_i64::<BE>(
    ...
    // Process stored character sequences
    context_rule!(
        term,
        buf,
        nametable,
